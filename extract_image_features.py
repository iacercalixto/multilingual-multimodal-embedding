...
def get_cnn_features(image_list, split, batch_size, dataset_name, pretrained_cnn, pretrained_cnn_name):
    ...
    for start, end in zip(range(0, len(image_list)+batch_size, batch_size),
        ...
        gfeats_cpu = gfeats.data.cpu().numpy()
        #gfeats_cpu = gfeats_cpu.flatten()
        #print( "gfeats_cpu.shape: ", gfeats_cpu.shape )
        #local_features_storage.append(lfeats)
        global_features_storage.append(gfeats_cpu)

    print("Finished processing %d images" % len(global_features_storage))
    hdf5_file.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--images_path", type=str,
                        ...
                        default="/media/storage2tb/resources/flickr30k/flickr30k-images")
